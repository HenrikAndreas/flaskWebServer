...
from forms import Login, Lamp, Register
from idGenerator import idGenerator # Self made function for random userID
import os
...
# Function to add users to userdata.dat
def addToUserdata(user, password, userId):
    dataFile = open('userdata.dat', 'a')
    dataFile.write(f"{user}|{password}|{userId}\n")
    dataFile.close()
...
def registration():
    ...
    if username in userData:
        return redirect('register') # Flash danger message!
    else:
        userID = idGenerator(24)
        addToUserdata(username, password, userID)
        return redirect('login')    #Flash successful message!
